...
void launch_fattn(
    ...
    const int nbatch_fa, const bool need_f16_K, const bool need_f16_V, const bool stream_k, const bool use_top_k,
    ...
    const int64_t n_kv_iter = use_top_k ? top_k->ne[0] : K->ne[1];
    const int ntiles_KV = (n_kv_iter + nbatch_fa - 1) / nbatch_fa; // Max. number of parallel blocks limited by KV cache length.
    ...
    const fattn_perf_hints perf_hints = {
        top_k ? (const int32_t *) top_k->data : nullptr,
        top_k ? (int32_t) top_k->ne[0] : 0,
    };
