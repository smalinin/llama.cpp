...
static __global__ void flash_attn_ext_f16(
    ...
    const int  * GGML_CUDA_RESTRICT KV_max         = use_sparse && mask_ptr == nullptr ? nullptr : KV_max_ptr;
    ...
    if (use_sparse && mask_ptr != nullptr) {
    ...
    while (kbc < kbc_stop && kb0_stop == iter_k) {
        ...
        const int32_t * indices = use_sparse ?
            sparse_indices + (mask_ptr != nullptr ?
                (int64_t(sequence % ne33)*iter_j + jt)*ne11 :
                (int64_t(sequence % ne33)*ne31 + jt*ncols1)*ne11) : nullptr;
