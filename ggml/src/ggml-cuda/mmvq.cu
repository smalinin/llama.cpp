...
static __global__ void mul_mat_vec_q(
    ...
    if constexpr (has_fusion) {
        ...
        gate_dst      = (float *) fusion.gate_dst;
...
#pragma unroll
    ...
    if (gate_dst) {
        gate_dst += sample_dst*stride_sample_dst + channel_dst*stride_channel_dst + row0;
    }
...
#pragma unroll
            ...
            if (threadIdx.x == i && (rows_per_cuda_block == 1 || uint32_t(row0 + i) < stride_col_dst)) {
                ...
                if constexpr (has_fusion) {
                    ...
                    if (use_gate) {
                        ...
                        if (gate_dst) {
                            gate_dst[j*stride_col_dst + i] = gate_value;
                        } else switch (active_glu) {
    ...
    if constexpr (!has_fusion) {
        GGML_UNUSED_VARS(use_gate, use_bias, use_gate_bias, use_scale, use_gate_scale, active_glu, glu_limit, gate_bias, gate_dst, x_bias, x_scale, gate_scale, tmp_gate);
    }
    if constexpr (type != GGML_TYPE_NVFP4) {
...
static __global__ void moe_down_q_reduction(
    ...
    const int row0 = c_rows_per_block * blockIdx.x;
    const int expert = ids[expert_slot];
    const int blocks_per_row_x = ncols_x / qk;
    constexpr int blocks_per_iter = vdr * warp_size / qi;
    const block_q8_1 * expert_y = vy + expert_slot * stride_channel_y;

    float values[c_rows_per_block] = { 0.0f };
#pragma unroll
    for (int i = 0; i < c_rows_per_block; ++i) {
        ...
        for (int kbx = threadIdx.x / (qi/vdr); kbx < blocks_per_row_x; kbx += blocks_per_iter) {
            const int kby = kbx * (qk/QK8_1);
            const int kqs = vdr * (threadIdx.x % (qi/vdr));
            values[i] += vec_dot_q_cuda(vx_ptr, &expert_y[kby], kbx_offset + kbx, kqs);
        }
...
#pragma unroll
        for (int i = 0; i < c_rows_per_block; ++i) {
            expert_values[expert_slot][i] = values[i];
        }
    }
    __syncthreads();

    if (threadIdx.y == 0 && threadIdx.x < c_rows_per_block) {
...
bool ggml_cuda_moe_down_q_reduction_supported(
        const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * ids,
        const ggml_tensor * weights, const ggml_tensor * dst) {
    static const bool disabled = [] {
        const char * value = std::getenv("GGML_CUDA_MOE_DOWN_REDUCE");
        return value != nullptr && std::atoi(value) == 0;
    }();
    if (disabled) {
        return false;
    }
    if (src0 == nullptr || src1 == nullptr || ids == nullptr || weights == nullptr || dst == nullptr) {
        return false;
    }
...
void ggml_cuda_moe_down_q_reduction(
    ...
    GGML_ASSERT(ggml_cuda_moe_down_q_reduction_supported(src0, src1, ids, weights, dst));

    cudaStream_t stream = ctx.stream();
    const int64_t ncols_padded = GGML_PAD(src1->ne[0], MATRIX_ROW_PADDING);
    ggml_cuda_pool_alloc<char> src1_q8_1(
        ctx.pool(), src1->ne[1] * ncols_padded * sizeof(block_q8_1) / QK8_1);
