...
def _v41_is_prime(n: int) -> bool:
    """Test primality without adding a sympy dependency."""
    if n < 2:
        return False
...
def _v41_find_next_prime(start: int, seen_primes: set[int]) -> int:
    """Return the first unused prime after start."""
    candidate = start + 1
...
class DeepseekV41Model(DeepseekV4Model):
    ...
    def _v41_flatten_hparams(self):
        """Promote text_config before the V4 tensor indexer reads hparams."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._v41_engram_tmp_paths: list[Path] = []
        logger.info(
    ...
    def set_gguf_parameters(self):
        ...
        if (engram_ids := hparams.get("engram_layer_ids")) is not None:
            ...
            expected_vocab = hparams["engram_compressed_vocab_size"]
            ...
            pad_token_id = hparams.get("engram_pad_token_id", 2)
    ...
    # One million rows use about 1 GB of float32 scratch.
    _V41_ENGRAM_CHUNK_ROWS = 1_000_000

    def _write_engram_table(self, bid: int) -> list[str]:
        """Quantize an engram table by row blocks into a disk-backed array."""
        ...
        with _safe_open(str(shard), framework="pt") as f:
            ...
            tmp_file = _tempfile.NamedTemporaryFile(
                prefix=f"llama-v41-engram-{bid}-", suffix=f"-{qtype.name}.bin", dir=tmp_dir, delete=False
            )
            tmp_path = tmp_file.name
            tmp_file.close()
            self._v41_engram_tmp_paths.append(Path(tmp_path))
            logger.info(
    ...
    def write(self):
        try:
            ...
            for path in self._v41_engram_tmp_paths:
