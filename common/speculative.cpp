...
struct common_speculative_impl_draft_eagle3 : public common_speculative_impl {
    ...
    llama_pos get_pos_max(llama_seq_id seq_id) const override {
        return need_boundary_stash() && seq_id >= 0 && seq_id < (llama_seq_id) n_seq ? pending_pos_last[seq_id] : -1;
    }
};
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    common_speculative_impl_draft_dflash(const common_params_speculative & params, uint32_t n_seq,
        ...
        , profile(is_dspark && std::getenv("LLAMA_DSPARK_PROFILE") != nullptr && std::atoi(std::getenv("LLAMA_DSPARK_PROFILE")) != 0)
    {
        auto * ctx_tgt = this->params.ctx_tgt;
        auto * ctx_dft = this->params.ctx_dft;
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        const int64_t t_graph_start = profile ? ggml_time_us() : 0;
        int ret = llama_decode(ctx_dft, batch);
        if (ret != 0) {
        ...
        if (profile) {
            ...
            SPC_INF("DSPARK_PROFILE stage=graph n_tokens=%d time_us=%" PRId64 "\n",
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (is_dspark) {
                ...
                // Profiling also reads confidence with p_min=0, but never uses it to change the draft.
                const bool measure_confidence = profile || dp.confidence != nullptr;
                const int64_t t_conf_start = measure_confidence ? ggml_time_us() : 0;
                const bool need_confidence = params.p_min > 0.0f || profile || dp.confidence != nullptr;
                ...
                if (dp.confidence && profile) {
                    SPC_INF("DSPARK_PROFILE stage=confidence seq=%d context=%d values=%zu time_us=%" PRId64 "\n",
                            (int) seq_id, (int) dp.n_past, confidence.size(), confidence_us);
                }
                const int64_t t_select_start = profile ? ggml_time_us() : 0;
                int32_t cutoff = -1;
                for (int32_t i = i_draft_beg; i < n_block_tokens; ++i) {
                ...
                if (profile) {
                    ...
                    SPC_INF("DSPARK_PROFILE stage=select seq=%d context=%d requested=%d produced=%zu "
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    ggml_context_ptr        backend_h_ctx;
    ggml_backend_buffer_ptr backend_h_buf;
    ggml_tensor *           backend_h = nullptr;
    bool                    backend_h_enabled = false;
    bool                    backend_h_requested = false;
    int32_t                 backend_h_batch_rows = 0;

    int32_t backend_h_target_base() const {
        return 0;
    }

    int32_t backend_h_pending_base() const {
        return backend_h_batch_rows;
    }

    int32_t backend_h_chain_base() const {
        return backend_h_pending_base() + n_seq;
    }

    int32_t backend_h_stage_base() const {
        return backend_h_chain_base() + n_seq*(params.n_max + 1);
    }

    int32_t backend_h_pending_row(llama_seq_id seq_id) const {
        return backend_h_pending_base() + seq_id;
    }

    int32_t backend_h_chain_row(llama_seq_id seq_id, int32_t step) const {
        GGML_ASSERT(step >= 0 && step <= params.n_max);
        return backend_h_chain_base() + seq_id*(params.n_max + 1) + step;
    }
        ...
        , params(params.draft)
    {
        ...
        llama_set_embeddings_nextn(ctx_tgt, true, /*masked*/ false);
        llama_set_embeddings_nextn(ctx_dft, true, /*masked*/ true);
        if (backend_h_requested) {
            llama_set_embeddings_nextn_host(ctx_tgt, false);
    ...
    ~common_speculative_impl_draft_mtp() override {
        auto * ctx_dft = this->params.ctx_dft;
        if (ctx_dft && backend_h_requested) {
            llama_set_embeddings_nextn_host(ctx_dft, true);
        }
        if (this->params.ctx_tgt && backend_h_requested) {
            llama_set_embeddings_nextn_host(this->params.ctx_tgt, true);
        }
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) backend_chains.size(); ++seq_id) {
            if (backend_chains[seq_id] == nullptr) {
                continue;
    ...
    void batch_use_hidden_tensor(ggml_tensor * src, int32_t row) {
        GGML_ASSERT(src && row >= 0 && batch.n_tokens > 0);
        GGML_ASSERT(row + batch.n_tokens <= ggml_nrows(src));
        GGML_ASSERT(src->type == GGML_TYPE_F32 && src->ne[0] == n_embd);
        GGML_ASSERT(src->nb[1] == ggml_row_size(src->type, src->ne[0]));

        batch_embd_h_tensor = *src;
        batch_embd_h_tensor.ne[1] = batch.n_tokens;
        batch_embd_h_tensor.ne[2] = 1;
        batch_embd_h_tensor.ne[3] = 1;
        batch_embd_h_tensor.nb[2] = batch_embd_h_tensor.nb[1] * batch.n_tokens;
        batch_embd_h_tensor.nb[3] = batch_embd_h_tensor.nb[2];
        batch_embd_h_tensor.data = static_cast<char *>(src->data) + (size_t) row * src->nb[1];
        batch_embd_h_tensor.view_src = nullptr;
        batch_embd_h_tensor.view_offs = 0;

        batch.embd_h = nullptr;
        batch.embd_h_tensor = &batch_embd_h_tensor;
    }

    bool ensure_backend_h(ggml_tensor * src) {
        if (backend_h) {
            return true;
        }
        if (!src || !src->buffer) {
        ...
        backend_h_ctx.reset(ggml_init(iparams));
        if (!backend_h_ctx) {
            return false;
        }

        const int32_t n_rows = backend_h_stage_base() + backend_h_batch_rows;
        backend_h = ggml_new_tensor_2d(backend_h_ctx.get(), GGML_TYPE_F32, n_embd, n_rows);
        auto * buft = ggml_backend_buffer_get_type(src->buffer);
        backend_h_buf.reset(ggml_backend_alloc_ctx_tensors_from_buft(backend_h_ctx.get(), buft));
        if (!backend_h_buf) {
            backend_h = nullptr;
            backend_h_ctx.reset();
            return false;
    ...
    void backend_h_set(int32_t row, const float * src) {
        GGML_ASSERT(backend_h && row >= 0 && row < backend_h->ne[1]);
        ggml_backend_tensor_set(backend_h, src, (size_t) row * backend_h->nb[1], (size_t) n_embd * sizeof(float));
    }

    void backend_h_copy(int32_t dst_row, const ggml_tensor * src, int32_t src_row, int32_t n_rows = 1) {
        GGML_ASSERT(backend_h && src);
        GGML_ASSERT(dst_row >= 0 && n_rows > 0 && dst_row + n_rows <= backend_h->ne[1]);
        GGML_ASSERT(src_row >= 0 && src_row + n_rows <= ggml_nrows(src));
        ...
        src_view.data = static_cast<char *>(src->data) + (size_t) src_row * src->nb[1];
        src_view.view_src = nullptr;
        src_view.view_offs = 0;

        ggml_tensor dst_view = *backend_h;
        dst_view.ne[1] = n_rows;
        dst_view.ne[2] = dst_view.ne[3] = 1;
        dst_view.nb[2] = dst_view.nb[1] * n_rows;
        dst_view.nb[3] = dst_view.nb[2];
        dst_view.data = static_cast<char *>(backend_h->data) + (size_t) dst_row * backend_h->nb[1];
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        const int32_t backend_target_row = backend_h_target_base();
        const int32_t backend_stage_row  = backend_h_stage_base();

        ggml_tensor * target_h_tensor = nullptr;
        if (backend_h_requested) {
            llama_synchronize(ctx_tgt);
            target_h_tensor = llama_get_embeddings_nextn_tensor(ctx_tgt);
            if (!target_h_tensor || ggml_nrows(target_h_tensor) != n_tokens || !ensure_backend_h(target_h_tensor)) {
                SPC_ERR("backend target MTP h output is unavailable (expected %d rows, got %d)\n",
                        n_tokens, target_h_tensor ? (int) ggml_nrows(target_h_tensor) : 0);
                return false;
            }
            backend_h_copy(backend_target_row, target_h_tensor, 0, n_tokens);
        }
        ...
        if (!is_mem_shared) {
            ...
            // to obtain logits. Do not feed overlapping positions into the draft
            // context again. Their freshly computed target h row is still used as
            // the boundary embedding for the first genuinely new draft position.
            for (int k = 0; k < n_tokens; ++k) {
                const llama_seq_id seq_id = batch_in.seq_id[k][0];
                ...
                if (is_media || pos > dft_pos[seq_id]) {
                    ...
                    if (h_prev == nullptr && h_src < 0) {
                        SPC_ERR("ctx_dft sequence %d is missing MTP boundary h at pos=%d (stored=%d)\n",
                                (int) seq_id, (int) pos - 1, (int) pending_pos[seq_id]);
                        return false;
                    }

                    if (catchup_pos_beg[seq_id] < 0) {
            ...
            if (backend_h_enabled && batch.n_tokens > 0) {
                GGML_ASSERT(catchup_h_src.size() == (size_t) batch.n_tokens);
                int32_t dst = 0;
                while (dst < batch.n_tokens) {
                    const int32_t src = catchup_h_src[dst];
                    int32_t n_copy = 1;
                    while (dst + n_copy < batch.n_tokens && src >= 0 &&
                            catchup_h_src[dst + n_copy] == src + n_copy) {
                        ++n_copy;
                    }
                    backend_h_copy(backend_stage_row + dst, backend_h, src, n_copy);
                    dst += n_copy;
                }
            ...
            for (int head = 0; head < n_mtp_layers; ++head) {
                if (chain_heads) {
                    ...
                    for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                        ...
                        llama_memory_seq_rm(mem_dft, seq_id, catchup_pos_beg[seq_id], -1);
                    }
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            if (backend_h_enabled) {
                backend_h_copy(backend_h_pending_row(seq_id), backend_h,
                        backend_target_row + i_batch_end[seq_id]);
            } else {
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            common_batch_add(batch, dp.id_last, dp.n_past, { seq_id }, true);
            if (backend_h_enabled) {
                backend_h_copy(backend_stage_row + batch.n_tokens - 1, backend_h,
                        backend_h_pending_row(seq_id));
            } else {
        ...
        while (n_drafting > 0) {
            ...
            std::vector<int32_t> h_out_row(n_seq, -1);
            int32_t n_h_out = 0;
            for (int32_t j = 0; j < batch.n_tokens; ++j) {
                if (batch.logits[j]) {
                    GGML_ASSERT(batch.n_seq_id[j] == 1);
                    h_out_row[batch.seq_id[j][0]] = n_h_out++;
                }
            }
            ...
            if (backend_h_enabled) {
                llama_synchronize(ctx_dft);
                h_tensor = llama_get_embeddings_nextn_tensor(ctx_dft);
                if (!h_tensor || ggml_nrows(h_tensor) != n_h_out) {
                    SPC_ERR("backend MTP h output is unavailable (expected %d rows, got %d)\n",
                            n_h_out, h_tensor ? (int) ggml_nrows(h_tensor) : 0);
                    n_drafting = 0;
                    break;
                }
            }
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                auto * smpl = smpls[seq_id].get();
                const int32_t h_src_row = h_out_row[seq_id];
                GGML_ASSERT(h_src_row >= 0);

                common_sampler_sample(smpl, ctx_dft, i_last[seq_id], true);
                const float * h_row = backend_h_enabled ? nullptr : llama_get_embeddings_nextn_ith(ctx_dft, i_last[seq_id]);
                ...
                if (chain_heads) {
                    ...
                    if (backend_h_enabled) {
                        if (result.size() == 1) {
                            backend_h_copy(backend_h_chain_row(seq_id, 0), backend_h,
                                    backend_h_pending_row(seq_id));
                        }
                        backend_h_copy(backend_h_chain_row(seq_id, result.size()), h_tensor, h_src_row);
                    } else {
                    ...
                    if (backend_h_enabled) {
                        backend_h_copy(backend_stage_row + batch.n_tokens - 1, h_tensor, h_src_row);
                    } else {
    ...
    void accept(llama_seq_id seq_id, uint16_t n_accepted, bool is_other) override {
        ...
        if (backend_h_enabled) {
            backend_h_copy(backend_h_pending_row(seq_id), backend_h,
                    backend_h_target_base() + i_batch_beg[seq_id] + i_h);
        } else {
            const size_t row_bytes = (size_t) n_embd * sizeof(float);
...
llama_pos common_speculative_get_pos_max(const common_speculative * spec, llama_seq_id seq_id) {
    ...
    llama_pos result = -1;
    for (const auto & impl : spec->impls) {
        result = std::max(result, impl->get_pos_max(seq_id));
    }
