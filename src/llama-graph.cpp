...
void llm_graph_result::set_outputs(const llm_graph_params & params) {
    ...
    if (t_mtp_top_k != nullptr) {
        ggml_set_output(t_mtp_top_k);
    }
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        ggml_flash_attn_ext_add_top_k_hint(cur, top_k);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (!kq_mask_top_k) {
        kq_mask_top_k = top_k ? build_attn_kq_mask_top_k(inp, top_k, il) : kq_mask;
    }
...
ggml_tensor * llm_graph_context::build_attn_kq_mask_top_k(
        llm_graph_input_attn_k_dsa * inp,
        ggml_tensor * top_k,
                int   il) const {
...
void llm_graph_context::build_mtp_device_draft() const {
    ...
    GGML_ASSERT(arch == LLM_ARCH_GLM_DSA);
